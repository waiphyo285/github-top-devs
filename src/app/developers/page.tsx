...
export default async function DevelopersPage({ searchParams }: PageProps) {
  ...
  const SortLink = ({ column, label }: { column: typeof sortBy; label: string }) => {
  ...
  return (
    <div className="space-y-8 pb-12">
          ...
          <form method="GET" className="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div className="md:col-span-4">
              <Select name="country" defaultValue={country}>
                <SelectTrigger className="w-full bg-secondary/60 border border-border/60 rounded-lg py-2.5 pl-3.5 pr-2.5 h-10 text-sm text-foreground hover:bg-secondary/80 focus:ring-1 focus:ring-primary focus:border-primary cursor-pointer transition-colors">
                  <SelectValue placeholder="All Countries" />
                </SelectTrigger>
                <SelectContent showSearch={true} searchPlaceholder="Search countries..." className="max-h-72">
                  <SelectItem value="">All Countries</SelectItem>
                  {countries.map((c) => (
                    <SelectItem key={c.country} value={c.country}>
                      {c.geoName} ({c.developerCount.toLocaleString()} devs)
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="md:col-span-2 flex gap-2">
              <Button type="submit" className="flex-1 text-xs py-3 bg-primary text-primary-foreground hover:bg-primary/90 font-bold rounded-lg shadow-sm">
                Apply Filters
              </Button>
              {(search || country) && (
                <Link
                  ...
                  className={buttonVariants({
                    variant: 'outline',
                    size: 'default',
                    className: 'border-border hover:bg-secondary text-xs flex items-center justify-center px-3',
                  })}
                >
                  Reset
                </Link>
      ...
      <div className="overflow-x-auto border border-border/40 rounded-2xl bg-card/15 shadow-[0_4px_30px_rgba(0,0,0,0.1)] backdrop-blur-md">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-border/30 bg-card/30 text-xs font-mono uppercase tracking-wider text-muted-foreground">
              ...
              <th className="py-4 px-6 font-medium w-36 text-right">
                <SortLink column="followers" label="Followers" />
              </th>
              <th className="py-4 px-6 font-medium w-40 text-right">
                <SortLink column="publicContributions" label="Contributions" />
              </th>
              <th className="py-4 px-6 font-medium w-36 text-right">
                <SortLink column="score" label="Score" />
              </th>
          ...
          <div className="flex space-x-2">
            {page > 1 ? (
              ...
              <Button
                ...
                size="sm"
                disabled
                className="border-border disabled:opacity-50 flex items-center gap-1 cursor-not-allowed"
              >
                <ChevronLeft className="h-4 w-4" />
                <span>Prev</span>
              </Button>
              ...
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                ...
                return isCurrent ? (
                  <Button
                    ...
                    variant="default"
                    size="sm"
                    className="h-9 w-9 p-0 font-mono bg-primary text-primary-foreground font-bold"
                  >
                    <span>{p}</span>
                  </Button>
                ) : (
                  <Link
                    ...
                    className={buttonVariants({
                      variant: 'outline',
                      size: 'sm',
                      className: 'h-9 w-9 p-0 font-mono border-border hover:bg-secondary flex items-center justify-center',
                    })}
                  >
                    {p}
                  </Link>
